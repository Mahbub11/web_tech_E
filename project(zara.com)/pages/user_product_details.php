...
?>
  ...
  $sql = $dbconn->query("SELECT * FROM comment WHERE product_id='$prod_id'");
  $product_count = mysqli_num_rows ($sql);
  if ($product_count > 0) {
        while($row = $sql->fetch_assoc()) {
            $id = $row['id'];
            $product_name = $row ['comment'];
  ...
  ?>
		...
		<form action="comment.php?prod_id=<?php echo $prod_id?>" method="post" enctype="multipart/form-data">
			<textarea placeholder="Add Your Comment" name="post"></textarea>
			<div >
